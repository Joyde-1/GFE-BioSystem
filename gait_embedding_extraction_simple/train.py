...
from gait_embedding_extraction_simple.gait_embedding_extraction_utils import load_config, browse_path, select_device, create_dataloader, select_model, evaluate
...
from gait_embedding_extraction_simple.loss_function_class.loss import ArcFaceLoss, BatchHardTripletLoss
...
def prepare_training_process(gait_embedding_extraction_config, model, train_dl):
    ...
    arcface_criterion = ArcFaceLoss(scale=gait_embedding_extraction_config.loss_function.arcface.scale, 
                                    margin=gait_embedding_extraction_config.loss_function.arcface.margin)
    triplet_criterion = BatchHardTripletLoss(margin=gait_embedding_extraction_config.loss_function.triplet.margin, 
                                             squared=False)
    ...
    return arcface_criterion, triplet_criterion, optimizer, scheduler

def train_one_epoch(gait_embedding_extraction_config, model, dataloader, arcface_criterion, triplet_criterion, optimizer, scheduler, device, epoch, experiment):
    """
    ...
    for i, batch in enumerate(tqdm(dataloader, desc=f"Training Epoch {epoch+1}")):
        ...
        # Forward pass
        embeddings, logits = model(keypoints_sequences, labels)
    ...
    # Average loss
    train_metrics['loss'] = running_loss / num_samples
    train_metrics['accuracy'] = running_accuracy / num_samples
...
def training_loop(gait_embedding_extraction_config, model, device, train_dl, val_dl, arcface_criterion, triplet_criterion, optimizer, scheduler, experiment):
    """
    ...
    for epoch in range(gait_embedding_extraction_config.training.epochs):
        ...
        # Train for one epoch
        train_metrics = train_one_epoch(gait_embedding_extraction_config, model, train_dl, arcface_criterion, triplet_criterion, optimizer, scheduler, device, epoch, experiment)
        
        # Validate model
        val_metrics = evaluate(gait_embedding_extraction_config, model, val_dl, arcface_criterion, triplet_criterion, device, epoch)
        ...
        print(f"Loss: {val_metrics['loss']:.4f}",
              ...
              f"Average Inter Distances: {val_metrics['avg_inter_dist']}", sep="\n")
...
if __name__ == "__main__":
    ...
    gait_embedding_extraction_config = load_config('gait_embedding_extraction_simple/config/gait_embedding_extraction_config.yaml')
    ...
    arcface_criterion, triplet_criterion, optimizer, scheduler = prepare_training_process(gait_embedding_extraction_config, model, train_dl)

    # Train model
    best_model = training_loop(gait_embedding_extraction_config, model, device, train_dl, val_dl, arcface_criterion, triplet_criterion, optimizer, scheduler, experiment)
