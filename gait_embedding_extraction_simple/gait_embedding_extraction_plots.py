...
def plot_data_splitting(gait_embedding_extraction_config, train, test, val=[]):
    ...
    data_split_dir = os.path.join(gait_embedding_extraction_config.save_path, "gait", "embedding_extraction", "data_split_plot")
...
def plot_tsne(gait_embedding_extraction_config, embs_2d, all_labels, epoch):
    plt.figure(figsize=(6, 6))
    plt.scatter(embs_2d[:, 0], embs_2d[:, 1], c=all_labels, cmap='tab20', s=5, alpha=0.8)
    plt.title(f"TSNE Val Embeddings Epoch {epoch:03d}")
    plt.xticks([]); plt.yticks([]); plt.tight_layout()

    tsne_plot_dir = os.path.join(gait_embedding_extraction_config.save_path, "gait", "embedding_extraction", "tsne_plot")

    # Crea la directory se non esiste
    if not os.path.exists(tsne_plot_dir):
        try:
            os.makedirs(tsne_plot_dir)
        except OSError as e:
            print(f"Error: {e.strerror}")
            return False
    
    tsne_plot_path = os.path.join(tsne_plot_dir, f"tsne_plot_epoch_{epoch}.png")

    if gait_embedding_extraction_config.save_image.plot_tsne:
        plt.savefig(tsne_plot_path)

    plt.close()

    if gait_embedding_extraction_config.show_image.plot_tsne:
        open_image(tsne_plot_path)