...
from yolo_pose_detection.yolo_pose_plots import plot_prediction
...
def select_device(yolo_pose_detection_config):
    """
    ...
    if yolo_pose_detection_config.training.device == "mps" and torch.backends.mps.is_available():
        device = "mps"
    elif yolo_pose_detection_config.training.device == "cuda" and torch.cuda.is_available():
        device = "cuda"
    else:
        device = "cpu"
...
def select_model(yolo_pose_detection_config):
    """
    ...
    Notes
    -----
    Currently supports 'yolo_pose11n' model type as specified in the configuration. If an unsupported model
    ...
    if yolo_pose_detection_config.training.model_name == 'yolo11n-pose' or yolo_pose_detection_config.training.model_name == 'yolo11s-pose' or yolo_pose_detection_config.training.model_name == 'yolo11m-pose':
        # Initialize model with the provided configuration
        model = YOLO(yolo_pose_detection_config.model_weights_path + f'/{yolo_pose_detection_config.training.model_name}.pt', verbose=True)

    # If no valid model type is specified, print an error message and exit
    if model is None:
        print("Model name is not valid. Check yolo_pose_detection_config.yaml")
        sys.exit(1)
...
def evaluate_model(yolo_pose_detection_config, biometric_trait, model, device):
    """
    ...
    results = model.val(
        data=yolo_pose_detection_config.train_yolo_pose_model_path,
        imgsz=yolo_pose_detection_config.data.image_size,                   # Dimensione immagine
        batch=yolo_pose_detection_config.training.batch_size,               # Dimensione batch
        save_json=True,
        ...
        plots=True,
        split='test',
        project=f'{yolo_pose_detection_config.save_path}/{biometric_trait}/yolo_pose/test',
        name=yolo_pose_detection_config.training.model_name,
        exist_ok=True
    )

    return results

def plot_inferences(yolo_pose_detection_config, biometric_trait, model, device):
    """
    ...
    images_path = os.path.join(yolo_pose_detection_config.save_data_splitted_path, "splitted database", biometric_trait, "test", "images")
    bounding_boxes_path = os.path.join(yolo_pose_detection_config.save_data_splitted_path, "splitted database", biometric_trait, "test", "labels")
    # images_path = os.path.join(detection_yolo_pose_config.data_dir, biometric_trait)
    # bounding_boxes_path = os.path.join(detection_yolo_pose_config.bounding_boxes_dir, biometric_trait)
    ...
    for image_file, bounding_box_file in tqdm(zip(image_files, bounding_box_files), desc="Plotting inference images", unit=" plot"):
        if image_file.endswith(".bmp") and bounding_box_file.endswith(".txt"):
            ...
            prediction = model.predict(
                ...
                conf=0.4,
                iou=0.7,
                imgsz=yolo_pose_detection_config.data.image_size,
                device=device,
                retina_masks=True,
            ...
            plot_prediction(yolo_pose_detection_config, biometric_trait, reference_image, prediction_image, acquisition_name)