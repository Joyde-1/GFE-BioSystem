...
class EarMatching:
    ...
    def _compare_templates(self, template1, template2):
        ...
        Ritorna:
            float: distanza (score) tra i due template. Valori minori indicano una maggiore somiglianza.
        """
        if self.ear_config.matching_algorithm == 'chi-square':
            return self._chi_square(template1, template2)
        elif self.ear_config.matching_algorithm == 'hamming':
            return self._hamming_distance(template1, template2)
        elif self.ear_config.matching_algorithm == 'euclidean':
            return self._euclidean_distance(template1, template2)
        elif self.ear_config.matching_algorithm == 'cosine_similarity':
    ...
    def calculate_far(self, subjects):
        ...
        with open(self.ear_config.results_path + f"/ear_results_FAR", "w") as file:
                ...
                for i in range(num_acquisizioni):
                    ...
                    for other_subject in subjects:
                        ...
                        for j in range(len(other_templates)):
                            ...
                            if matching_score < self.ear_config.matching.threshold:
                                FA += 1
    ...
    def calculate_frr(self, subjects):
        ...
        # Opening the text file to save the matching results
        with open(self.ear_config.results_path + f"/ear_results_FRR", "w") as file:
            # Per ogni soggetto
            for subject in subjects:
                templates = subjects[subject]['template']
    ...
    def calculate_roc_and_det(self, subjects):
        ...
        plt.savefig(self.ear_config.results_path + f"/ear_ROC.png")
        plt.close()
        ...
        # Salva il grafico
        plt.savefig(self.ear_config.results_path + f"/ear_DET.png")
        plt.close()
    ...
    def far_vs_frr(self, subjects):
        ...
        original_threshold = self.ear_config.matching.threshold

        # Loop sul range di threshold
        for th in thresholds:
            # Aggiorna la threshold nel file di configurazione
            self.ear_config.matching.threshold = th
        ...
        self.ear_config.matching.threshold = original_threshold
        ...
        # Salva il grafico
        plt.savefig(self.ear_config.results_path + f"/ear_far_vs_frr.png")
        plt.close()