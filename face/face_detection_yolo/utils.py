...
try:
    from face.face_detection_yolo.plots import plot_prediction
except ModuleNotFoundError:
    try:
        from face_detection_yolo.plots import plot_prediction
    except ModuleNotFoundError:
        from multimodal.plots import plot_prediction
...
def select_device(face_detection_yolo_config):
    """
    ...
    if face_detection_yolo_config.training.device == "mps" and torch.backends.mps.is_available():
        device = "mps"
    elif face_detection_yolo_config.training.device == "cuda" and torch.cuda.is_available():
        device = "cuda"
    else:
        device = "cpu"
...
def select_model(face_detection_yolo_config):
    """
    ...
    if face_detection_yolo_config.training.model_name == 'yolo11n' or face_detection_yolo_config.training.model_name == 'yolo11s' or face_detection_yolo_config.training.model_name == 'yolo11m':
        # Initialize model with the provided configuration
        model = YOLO(face_detection_yolo_config.model_weights_path + f'/{face_detection_yolo_config.training.model_name}.pt', verbose=True)

    # If no valid model type is specified, print an error message and exit
    if model is None:
        print("Model name is not valid. Check face_detection_yolo_config.yaml")
        sys.exit(1)
...
def evaluate_model(face_detection_yolo_config, model, device):
    """
    ...
    results = model.val(
        data=face_detection_yolo_config.train_yolo_model_path,
        ...
        plots=True,
        split='test',
        project=f'{face_detection_yolo_config.save_path}/face/yolo/test',
...
def plot_inferences(face_detection_yolo_config, model, device):
    """
    ...
    images_path = os.path.join(face_detection_yolo_config.save_data_splitted_path, "splitted database", "face", "test", "images")
    bounding_boxes_path = os.path.join(face_detection_yolo_config.save_data_splitted_path, "splitted database", "face", "test", "labels")
    # images_path = os.path.join(face_detection_yolo_config.data_dir, "face")
    # bounding_boxes_path = os.path.join(face_detection_yolo_config.bounding_boxes_dir, "face")
    ...
    image_files = [f for f in os.listdir(images_path) if f.endswith('.bmp')]
    ...
    for image_file, bounding_box_file in tqdm(zip(image_files, bounding_box_files), desc="Plotting inference images", unit=" plot"):
        if image_file.endswith(".bmp") and bounding_box_file.endswith(".txt"):
            ...
            prediction = model.predict(
                ...
                conf=0.4,
                iou=0.7,
                imgsz=face_detection_yolo_config.data.image_size,
                device=device,
                retina_masks=True,
            ...
            plot_prediction(face_detection_yolo_config, reference_image, prediction_image, acquisition_name)