...
def plot_data_splitting(ear_landmarks_detection_config, train, test, val=[]):
    """
    ...
    data_split_dir = os.path.join(ear_landmarks_detection_config.save_path, "face", "cnn", "plots", "data split plot")
    ...
    if ear_landmarks_detection_config.save_image.plot_data_splitting:
        plt.savefig(data_split_path)
    
    plt.close()

    if ear_landmarks_detection_config.show_image.plot_data_splitting:
        open_image(data_split_path)

def plot_prediction(ear_landmarks_detection_config, reference_image, predicted_image, counter):
    """
    Plots the predicted and ground truth landmarks on the image.
    ...
    prediction_plots_dir = os.path.join(ear_landmarks_detection_config.save_path, "ear_landmarks_detection", ear_landmarks_detection_config.biometric_trait, "plots", "prediction_plots")
    ...
    if ear_landmarks_detection_config.save_image.plot_prediction:
        plt.savefig(prediction_plots_path)

    plt.close()

    if ear_landmarks_detection_config.show_image.plot_prediction:
        open_image(prediction_plots_path)

def plot_reference_vs_prediction(ear_landmarks_detection_config, test_image, counter):
    """
    Plots the predicted and ground truth landmarks on the image.
    ...
    plt.axis("off")
    plt.title("Reference vs Prediction Landmarks")

    plt.tight_layout()
    
    ref_vs_pred_plots_dir = os.path.join(ear_landmarks_detection_config.save_path,"ear_landmarks_detection", ear_landmarks_detection_config.biometric_trait, "plots", "reference_vs_prediction_plots")
    ...
    if ear_landmarks_detection_config.save_image.plot_reference_vs_prediction:
        plt.savefig(ref_vs_pred_plots_path)

    plt.close()

    if ear_landmarks_detection_config.show_image.plot_reference_vs_prediction:
