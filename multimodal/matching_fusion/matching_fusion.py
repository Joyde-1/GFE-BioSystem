...
class MatchingMultimodalFusion:
    ...
    def _compare_templates(self, gait_template1, face_template1, ear_dx_template1, ear_sx_template1,
                            gait_template2, face_template2, ear_dx_template2, ear_sx_template2):
        """
        ...
        ear_dx_score = self._euclidean_distance(ear_dx_template1, ear_dx_template2)
        ear_sx_score = self._euclidean_distance(ear_sx_template1, ear_sx_template2)

        weighted_gait_score = gait_score * self.multimodal_config.matching_fusion.weight_gait
        weighted_face_score = face_score * self.multimodal_config.matching_fusion.weight_face
        weighted_ear_dx_score = ear_dx_score * self.multimodal_config.matching_fusion.weight_ear
        weighted_ear_sx_score = ear_sx_score * self.multimodal_config.matching_fusion.weight_ear

        fused_score = weighted_gait_score + weighted_face_score + weighted_ear_dx_score + weighted_ear_sx_score

        return fused_score
        
    def calculate_far(self, subjects_gait, subjects_face, subjects_ear_dx, subjects_ear_sx):
        """
        ...
        with open(self.multimodal_config.results_path + f"/multimodal_matching_fusion_results_FAR", "w") as file:
            # Per ogni soggetto (che sarà la query)
            for subject_gait, subject_face, subject_ear_dx, subject_ear_sx in zip(subjects_gait, subjects_face, subjects_ear_dx, subjects_ear_sx):
                gait_templates = subjects_gait[subject_gait]['template']
                face_templates = subjects_face[subject_face]['template']
                ear_dx_templates = subjects_ear_dx[subject_ear_dx]['template']
                ear_sx_templates = subjects_ear_sx[subject_ear_sx]['template']
                ...
                for i in range(num_acquisizioni):
                    ...
                    query_ear_dx_template = ear_dx_templates[i]
                    query_ear_sx_template = ear_sx_templates[i]
                    
                    # Confronta la query con tutte le acquisizioni dei soggetti differenti
                    for other_subject_gait, other_subject_face, other_subject_ear_dx, other_subject_ear_sx in zip(subjects_gait, subjects_face, subjects_ear_dx, subjects_ear_sx):
                        if other_subject_gait == subject_gait and other_subject_face == subject_face and other_subject_ear_dx == subject_ear_dx and other_subject_ear_sx == subject_ear_sx:
                        ...
                        other_ear_dx_templates = subjects_ear_dx[other_subject_ear_dx]['template']
                        other_ear_sx_templates = subjects_ear_sx[other_subject_ear_sx]['template']
                        ...
                        for j in range(len(other_gait_templates)):
                            ...
                            matching_score = self._compare_templates(query_gait_template, query_face_template, query_ear_dx_template, query_ear_sx_template,
                                                                     other_gait_templates[j], other_face_templates[j], other_ear_dx_templates[j], other_ear_sx_templates[j])

                            if matching_score < self.multimodal_config.matching_fusion.threshold:
                                FA += 1
    ...
    def calculate_frr(self, subjects_gait, subjects_face, subjects_ear_dx, subjects_ear_sx):
        """
        ...
        # Opening the text file to save the matching results
        with open(self.multimodal_config.results_path + f"/multimodal_matching_fusion_results_FRR", "w") as file:
            # Per ogni soggetto
            for subject_gait, subject_face, subject_ear_dx, subject_ear_sx in zip(subjects_gait, subjects_face, subjects_ear_dx, subjects_ear_sx):
                gait_templates = subjects_gait[subject_gait]['template']
                face_templates = subjects_face[subject_face]['template']
                ...
                for i in range(K):
                    for j in range(i + 1, K):
                        ...
                        matching_score = self._compare_templates(gait_templates[i], face_templates[i], ear_dx_templates[i], ear_sx_templates[i],
                                                                 gait_templates[j], face_templates[j], ear_dx_templates[j], ear_sx_templates[j])
    ...
    def calculate_roc_and_det(self, subjects_gait, subjects_face, subjects_ear_dx, subjects_ear_sx):
        """
        ...
        for subject_gait, subject_face, subject_ear_dx, subject_ear_sx in zip(subjects_gait, subjects_face, subjects_ear_dx, subjects_ear_sx):
            ...
            for i in range(num_acquisizioni):
                for j in range(i + 1, num_acquisizioni):
                    score = self._compare_templates(gait_templates[i], face_templates[i], ear_dx_templates[i], ear_sx_templates[i],
                                                    gait_templates[j], face_templates[j], ear_dx_templates[j], ear_sx_templates[j])
        ...
        for subject_gait, subject_face, subject_ear_dx, subject_ear_sx in zip(subjects_gait, subjects_face, subjects_ear_dx, subjects_ear_sx):
            ...
            query_ear_dx_templates = subjects_ear_dx[subject_ear_dx]['template']
            query_ear_sx_templates = subjects_ear_sx[subject_ear_sx]['template']
            for i in range(len(query_face_templates)):
                for other_subject_gait, other_subject_face, other_subject_ear_dx, other_subject_ear_sx in zip(subjects_gait, subjects_face, subjects_ear_dx, subjects_ear_sx):
                    ...
                    other_ear_sx_templates = subjects_ear_sx[other_subject_ear_sx]['template']
                    for j in range(len(other_gait_templates)):
                        score = self._compare_templates(query_gait_templates[i], query_face_templates[i], query_ear_dx_templates[i], query_ear_sx_templates[i],
                                                        other_gait_templates[j], other_face_templates[j], other_ear_dx_templates[j], other_ear_sx_templates[j])
                        if score is not None:
                            scores.append(score)
        ...
        plt.savefig(self.multimodal_config.results_path + f"/multimodal_matching_fusion_ROC.png")
        plt.close()
        ...
        # Salva il grafico
        plt.savefig(self.multimodal_config.results_path + f"/multimodal_matching_fusion_DET.png")
        plt.close()

        return FAR_pct, GAR_pct, FRR_pct, EER_pct, thresholds

    def far_vs_frr(self, subjects_gait, subjects_face, subjects_ear_dx, subjects_ear_sx):
        ...
        original_threshold = self.multimodal_config.matching_fusion.threshold

        # Loop sul range di threshold
        for th in thresholds:
            # Aggiorna la threshold nel file di configurazione
            self.multimodal_config.matching_fusion.threshold = th
            
            # Calcola FAR e FRR sui soggetti (il metodo scrive anche su file, seppur tu possa ignorare questo aspetto per il grafico)
            far, _, _ = self.calculate_far(subjects_gait, subjects_face, subjects_ear_dx, subjects_ear_sx)
            frr, _, _ = self.calculate_frr(subjects_gait, subjects_face, subjects_ear_dx, subjects_ear_sx)
        ...
        self.multimodal_config.matching_fusion.threshold = original_threshold
        ...
        # Salva il grafico
        plt.savefig(self.multimodal_config.results_path + f"/multimodal_matching_fusion_far_vs_frr.png")
        plt.close()