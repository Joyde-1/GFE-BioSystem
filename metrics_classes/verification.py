...
class Verification:
    def __init__(self, config, biometric_trait):
        ...
        self.matcher = Matching(self._config)
    ...
    def calculate_far(self, subjects):
        ...
        - search_ms: tempo medio di ricerca per confronto (millisecondi)
        - throughput: confronti al secondo
        """
        ...
        with open(self._config.results_path + f"/{self.biometric_trait}_results_FAR", "w") as file:
                ...
                for i in range(num_acquisizioni):
                    ...
                    for other_subject in subjects:
                        ...
                        for j in range(len(other_templates)):
                            ...
                            matching_score = self.matcher.compare_templates(query_template, other_templates[j])
            ...
            search_ms = (elapsed / T_imp) * 1000 if T_imp else 0.0
            throughput = T_imp / elapsed if elapsed > 0 else 0.0
            FAR = (FA / T_imp) * 100 if T_imp > 0 else 0

        return FAR, FA, T_imp, search_ms, throughput
    ...
    def calculate_frr(self, subjects):
            ...
            for subject in subjects:
                ...
                for i in range(K):
                    for j in range(i + 1, K):
                        ...
                        matching_score = self.matcher.compare_templates(templates[i], templates[j])
        ...
        elapsed = time.perf_counter() - tic
        search_ms = (elapsed / T_legit) * 1000 if T_legit else 0.0
        throughput = T_legit / elapsed if elapsed > 0 else 0.0
        FRR = (FR / T_legit) * 100 if T_legit > 0 else 0

        return FRR, FR, T_legit, search_ms, throughput
        ...
        for subject in subjects:
            ...
            for i in range(num_acquisizioni):
                for j in range(i + 1, num_acquisizioni):
                    score = self.matcher.compare_templates(templates[i], templates[j])
        ...
        for subject in subjects:
            ...
            for i in range(len(query_templates)):
                for other_subject in subjects:
                    ...
                    other_templates = subjects[other_subject]['template']
                    for j in range(len(other_templates)):
                        score = self.matcher.compare_templates(query_templates[i], other_templates[j])
                        if score is not None:
                            scores.append(score)
    ...
    def far_vs_frr(self, subjects):
        ...
        for th in thresholds:
            ...
            far, fa, t_imp, ms_far, thr_far = self.calculate_far(subjects)
            frr, fr, t_legit, ms_frr, thr_frr = self.calculate_frr(subjects)
