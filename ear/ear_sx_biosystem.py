...
if __name__ == '__main__':
    ear_config = load_config('config/ear_sx_config.yaml')
    ...
    images, image_names, image_paths = images_data.load_images(ear_config, 'ear_sx')

    pre_processing = EarPreProcessing(ear_config)

    yolo = Yolo(ear_config, 'ear_sx')
    ...
    if ear_config.use_checkpoint:
        checkpoint = load_checkpoint('checkpoint_ear_sx.json')
    ...
    for current_index, (image, image_name, image_path) in enumerate(zip(images, image_names, image_paths)):
        ...
        if ear_config.save_image.pre_processed:
            save_image(ear_config, 'ear_sx', pre_processed_ear_image, image_name, 'pre_processed_ear_sx_image')

        if ear_config.detector == 'yolo':
            pre_processed_ear_image_path = path_extractor(ear_config, 'ear_sx', image_name, 'pre_processed_ear_sx_image')
        ...
        if ear_config.save_image.detected:
            save_image(ear_config, 'ear_sx', detected_image, image_name, 'detected_ear_sx_bounding_box')
        ...
        if ear_config.save_image.post_processed:
            save_image(ear_config, 'ear_sx', post_processed_ear_image, image_name, 'post_processed_ear_sx_image')
        ...
        if ear_config.features_extractor == 'fisherface' and ear_config.features_extraction.fisherfaces.load_model:
            ...
            if ear_config.use_checkpoint:
                save_checkpoint('checkpoint_ear_sx.json', current_index + 1)
        ...
        if ear_config.save_image.features_extracted:
            save_image(ear_config, 'ear_sx', ear_template_vis, image_name, f"{ear_config.features_extractor}_ear_sx_image")
    ...
    #             save_image(ear_config, 'ear_sx', padded_ear_images[i + padded_ear_images_index], f"{subject}_{i + 1}", "padded_ear_sx_image")
    ...
    if ear_config.features_extractor == 'fisherface' and not ear_config.features_extraction.fisherfaces.load_model:
        ...
        for subject in subjects.keys():
            ...
            if ear_config.save_image.features_extracted:
                for i in range(num_acquisitions):
                    save_image(ear_config, 'ear_sx', visual_fisherfaces[i + fisherfaces_index], f"{subject}_{i + 1}", f"{ear_config.features_extractor}_ear_sx_image")

            # Aggiorna l'indice per il prossimo soggetto
            fisherfaces_index += num_acquisitions

    # Verification phase
    ear_verification = Verification(ear_config, 'ear_sx')
    ...
    ear_recognition_closed_set = RecognitionClosedSet(ear_config, 'ear_sx')
    ...
    ear_recognition_open_set = RecognitionOpenSet(ear_config, 'ear_sx')
