...
from matching.matching_ear import MatchingFace
...
if __name__ == '__main__':
    ear_config = load_config('ear/config/ear_config.yaml')
    ...
    images_data = PrepareData(ear_config)
    images, image_names, image_paths = images_data.load_ear_images()
    ...
    for current_index, (image, image_name, image_path) in enumerate(zip(images, image_names, image_paths)):
        ...
        if ear_config.save_image.pre_processed_ear_image:
            save_image(ear_config, pre_processed_ear_image, image_name, "pre_processed_ear_image")
        ...
        if ear_config.ear_detection.detector == 'viola-jones':
            ...
            pre_processed_ear_image_path = path_extractor(ear_config, image_name, "pre_processed_ear_image")
            detected_image, bounding_box = yolo.predict_ear_bounding_box(pre_processed_ear_image_path)
        else:
            raise ValueError("Unknown algorithm type! \n")
        
        if ear_config.save_image.detected_ear_bounding_box:
            save_image(ear_config, detected_image, image_name, "detected_ear_bounding_box")

        post_processed_ear_image = post_processing.post_processing_image(pre_processed_ear_image.copy(), bounding_box)

        if ear_config.save_image.post_processed_ear_image:
            save_image(ear_config, post_processed_ear_image, image_name, "post_processed_ear_image")
        ...
        if ear_config.save_image.features_extracted_ear_image:
            save_image(ear_config, ear_template_vis, image_name, f"{ear_config.features_extractor}_ear_image")
        ...
        for subject in subjects.keys():
            ...
            if ear_config.save_image.features_extracted_ear_image:
                for i in range(num_acquisitions):
                    save_image(ear_config, visual_fisherfaces[i + fisherfaces_index], f"{subject}_{i}", f"{ear_config.features_extractor}_ear_image")

            # Aggiorna l'indice per il prossimo soggetto
            fisherfaces_index += num_acquisitions
    
    matching_ear = MatchingFace(ear_config)

    far, fa, t_imp = matching_ear.calculate_far(subjects)
    frr, fr, t_legit = matching_ear.calculate_frr(subjects)
    accuracy = matching_ear.calculate_accuracy(t_imp, t_legit, fa, fr)

    print("", "Matching ear metrics:")
    print(f"FAR: {far:.4f} %")
    print(f"FRR: {frr:.4f} %")
    print(f"accuracy: {accuracy:.4f} %")

    matching_ear.calculate_roc_and_det(subjects)
    matching_ear.far_vs_frr(subjects)