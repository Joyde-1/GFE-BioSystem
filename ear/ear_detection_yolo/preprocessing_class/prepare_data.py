...
class PrepareData():
    """
    Prepares ear detection data for deep learning model.
    ...
    def __init__(self, face_detection_yolo_config):
        """
        ...
        self.face_detection_yolo_config = face_detection_yolo_config
    ...
    def _get_image_names(self):
        ...
        images_path = os.path.join(self.face_detection_yolo_config.data_dir, "face")
    ...
    def _data_splitting(self, data):
        ...
        train_set_image_names, test_set_image_names, train_set_subjects, _ = train_test_split(data['image_names'], data['subjects'], test_size=self.face_detection_yolo_config.data.test_size, random_state=42, shuffle=True, stratify=data['subjects'])
        train_set_image_names, val_set_image_names, _, _ = train_test_split(train_set_image_names, train_set_subjects, test_size=self.face_detection_yolo_config.data.val_size, random_state=42, shuffle=True, stratify=train_set_subjects)
    ...
    def _load_and_save_images_and_bounding_boxes(self, images_path, images_file, train_set_image_names, val_set_image_names):
        ...
        for subset in ['train', 'val', 'test']:
            subset_path = os.path.join(self.face_detection_yolo_config.save_data_splitted_path, "splitted database", "face", subset)
        ...
        for image_file in tqdm(images_file, desc=f"Loading face image files", unit="file"):
            if image_file.endswith(".bmp"):
                # Carica l'immagine
                image_path = os.path.join(images_path, image_file)
                ...
                bounding_box_path = os.path.join(self.face_detection_yolo_config.bounding_boxes_dir, "face", image_name + '.txt')
                ...
                image = Image.fromarray(image).resize((self.face_detection_yolo_config.data.image_size, self.face_detection_yolo_config.data.image_size))
                ...
                subset_image_path = os.path.join(self.face_detection_yolo_config.save_data_splitted_path, "splitted database", "face", subset, "images", f"{image_name}.bmp")
                subset_bounding_box_path = os.path.join(self.face_detection_yolo_config.save_data_splitted_path, "splitted database", "face", subset, "labels", f"{image_name}.txt")
