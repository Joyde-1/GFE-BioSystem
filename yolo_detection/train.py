...
def train_model(detection_yolo_config, biometric_trait, model, device, num_workers):
    """
    Executes the training loop for a given model configuration.

    Parameters
    ----------
    detection_yolo_config : object
    ...
    train_results = model.train(
        data=detection_yolo_config.train_yolo_model_path,              # File YAML che descrive il dataset
        epochs=detection_yolo_config.training.epochs,                  # Numero di epoche
        patience=detection_yolo_config.training.patience,
        batch=detection_yolo_config.training.batch_size,               # Dimensione batch
        imgsz=detection_yolo_config.data.image_size,                   # Dimensione immagine
        save=True,
        device=device,                                                      # Usa GPU (se disponibile)
        workers=num_workers,
        project=f'{detection_yolo_config.save_path}/{biometric_trait}/yolo/training',
        name=detection_yolo_config.training.model_name,
        exist_ok=True,
        # optimizer=,
        seed=42,
        deterministic=True,
...
def save_model(detection_yolo_config, biometric_trait):
    """
    ...
    model_path = os.path.join(detection_yolo_config.training.checkpoints_dir, f"{detection_yolo_config.training.model_name}.pt")

    # Save the best model's state dictionary to the checkpoint directory
    shutil.copyfile(f'{detection_yolo_config.save_path}/{biometric_trait}/yolo/training/{detection_yolo_config.training.model_name}/weights/best.pt', model_path)
...
if __name__ == '__main__':
    ...
    if detection_yolo_config.browse_path:
        ...
        detection_yolo_config.training.checkpoints_dir = browse_path('Select the folder where model checkpoint will be saved')
    ...
    experiment = Experiment(
        api_key=detection_yolo_config.comet.api_key,
        project_name=detection_yolo_config.comet.project_name,
        workspace=detection_yolo_config.comet.workspace,
        auto_metric_logging=False
    )

    experiment.set_name(detection_yolo_config.training.model_name)
    
    experiment.add_tag("yolo")

    experiment.log_parameters(detection_yolo_config.data)
    experiment.log_parameters(detection_yolo_config.training)
    ...
    model = select_model(detection_yolo_config)
    ...
    trained_model, train_results = train_model(detection_yolo_config, biometric_trait, model, device, num_workers)
    ...
    save_model(detection_yolo_config, biometric_trait)
