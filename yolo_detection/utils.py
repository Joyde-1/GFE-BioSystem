...
try:
    from yolo_detection.plots import plot_prediction
...
def select_device(detection_yolo_config):
    """
    ...
    if detection_yolo_config.training.device == "mps" and torch.backends.mps.is_available():
        device = "mps"
    elif detection_yolo_config.training.device == "cuda" and torch.cuda.is_available():
        device = "cuda"
    else:
        device = "cpu"
...
def select_model(detection_yolo_config):
    """
    ...
    if detection_yolo_config.training.model_name == 'yolo11n' or detection_yolo_config.training.model_name == 'yolo11s' or detection_yolo_config.training.model_name == 'yolo11m':
        # Initialize model with the provided configuration
        model = YOLO(detection_yolo_config.model_weights_path + f'/{detection_yolo_config.training.model_name}.pt', verbose=True)

    # If no valid model type is specified, print an error message and exit
    if model is None:
        print("Model name is not valid. Check detection_yolo_config.yaml")
        sys.exit(1)
...
def evaluate_model(detection_yolo_config, biometric_trait, model, device):
    """
    ...
    results = model.val(
        data=detection_yolo_config.train_yolo_model_path,
        ...
        plots=True,
        split='test',
        project=f'{detection_yolo_config.save_path}/{biometric_trait}/yolo/test',
...
def plot_inferences(detection_yolo_config, biometric_trait, model, device):
    """
    ...
    images_path = os.path.join(detection_yolo_config.save_data_splitted_path, "splitted database", biometric_trait, "test", "images")
    bounding_boxes_path = os.path.join(detection_yolo_config.save_data_splitted_path, "splitted database", biometric_trait, "test", "labels")
    ...
    for image_file, bounding_box_file in tqdm(zip(image_files, bounding_box_files), desc="Plotting inference images", unit=" plot"):
        if image_file.endswith(".bmp") and bounding_box_file.endswith(".txt"):
            ...
            prediction = model.predict(
                ...
                conf=0.4,
                iou=0.7,
                imgsz=detection_yolo_config.data.image_size,
                device=device,
                retina_masks=True,
            ...
            plot_prediction(detection_yolo_config, biometric_trait, reference_image, prediction_image, acquisition_name)